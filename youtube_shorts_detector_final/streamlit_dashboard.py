...
def show_report(video_id: str):
    ...
    reasoning_html = (data.get('reasoning_log', '—') or '—').replace('\n', '<br>')
    st.markdown(f"<div class='reasoning-box'>💬 {reasoning_html}</div>", unsafe_allow_html=True)
...
def show_admin_feedback():
    ...
    # ── 분석 영상 DB 관리 ────────────────────────────────────────
    st.markdown("<hr style='border-color:#f0f0f0;margin:40px 0 24px 0'>", unsafe_allow_html=True)

    # 토글 버튼
    if "show_db_manager" not in st.session_state:
        st.session_state.show_db_manager = False

    toggle_label = "🗑️ 분석 영상 DB 관리  ▲ 닫기" if st.session_state.show_db_manager else "🗑️ 분석 영상 DB 관리  ▼ 열기"
    if st.button(toggle_label, key="btn_toggle_db_manager"):
        st.session_state.show_db_manager = not st.session_state.show_db_manager
        st.session_state.del_page = 0   # 열 때마다 첫 페이지로
        st.rerun()

    if st.session_state.show_db_manager:

        st.markdown("""
        <div style="font-size:0.82rem;color:#888;margin:8px 0 20px 0">
          AnalysisResults · Contents 테이블 — 분석된 영상 목록 조회 및 삭제
        </div>
        """, unsafe_allow_html=True)

        # 분석 영상 목록 조회
        def load_analysis_videos(limit=200, cat_filter="전체", keyword=""):
            try:
                from database_manager import db_manager
                from database_models import AnalysisResults, Contents
                session = db_manager.get_session()
                try:
                    rows = session.query(AnalysisResults).join(
                        Contents, AnalysisResults.video_id == Contents.video_id, isouter=True
                    ).order_by(AnalysisResults.created_at.desc()).limit(limit).all()
                    result = []
                    for r in rows:
                        title = ""
                        ...
                        vid = r.video_id
                        if keyword and keyword.lower() not in vid.lower() and keyword.lower() not in title.lower():
                            continue
                        if cat_filter != "전체" and r.c_category != cat_filter:
                            continue
                        result.append({
                            "video_id":   vid,
                            "title":      (title[:45] + "…") if len(title) > 45 else (title or "—"),
                            "category":   r.c_category or "—",
                            "confidence": f"{r.confidence_score:.0%}" if r.confidence_score else "—",
                ...
                logger.error(f"분석 영상 목록 조회 실패: {e}")
                return [], False

        # 필터 행
        del_filter_cols = st.columns([2, 2, 1])
        with del_filter_cols[0]:
            del_cat_filter = st.selectbox(
                "카테고리 필터", ["전체", "C1", "C2", "C3", "C4", "C5"],
                key="del_cat_filter"
            )
        with del_filter_cols[1]:
            del_keyword = st.text_input("Video ID / 제목 검색", placeholder="검색어 입력", key="del_keyword")
        with del_filter_cols[2]:
            st.markdown("<div style='height:28px'></div>", unsafe_allow_html=True)
            if st.button("🔍 검색", key="btn_del_search", use_container_width=True):
                st.session_state.del_page = 0  # 검색 시 첫 페이지로

        video_list, db_ok = load_analysis_videos(
            cat_filter=del_cat_filter, keyword=del_keyword.strip()
        )

        if not db_ok:
            st.error("⚠️ DB 연결 실패 — 영상 목록을 불러올 수 없습니다.")
        elif not video_list:
            st.info("조건에 맞는 분석 영상이 없습니다.")
        else:
            PAGE_SIZE = 10
            total     = len(video_list)
            if "del_page" not in st.session_state:
                st.session_state.del_page = 0
            total_pages = max(1, (total + PAGE_SIZE - 1) // PAGE_SIZE)
            # 페이지 범위 보정
            st.session_state.del_page = max(0, min(st.session_state.del_page, total_pages - 1))
            page = st.session_state.del_page

            page_items = video_list[page * PAGE_SIZE : (page + 1) * PAGE_SIZE]

            # 상단 정보
            st.markdown(
                f"<div style='font-size:0.8rem;color:#888;padding-top:6px;margin-bottom:8px'>"
                f"총 <b style='color:#333'>{total}건</b> · "
                f"페이지 <b style='color:#333'>{page+1}</b> / {total_pages}</div>",
                unsafe_allow_html=True
            )

            CAT_COLOR = {"C1": "#ef4444", "C2": "#f97316", "C3": "#f59e0b",
                         "C4": "#8b5cf6", "C5": "#10b981"}

            # session_state 초기화
            if "del_checked" not in st.session_state:
                st.session_state.del_checked = {}

            page_ids = [v["video_id"] for v in page_items]

            # 전체선택 체크박스 헤더 행
            all_checked = all(st.session_state.del_checked.get(vid, False) for vid in page_ids)
            hcols = st.columns([0.5, 2.5, 3, 1, 1, 2, 1.2])
            select_all = hcols[0].checkbox("", value=all_checked, key="chk_all_page", label_visibility="collapsed")
            if select_all != all_checked:
                for vid in page_ids:
                    st.session_state.del_checked[vid] = select_all
                st.rerun()
            for col, label in zip(hcols[1:], ["Video ID", "제목", "분류", "신뢰도", "분석 시각", "삭제"]):
                col.markdown(
                    f"<div style='font-size:0.72rem;font-weight:700;color:#aaa;"
                    f"letter-spacing:0.08em;text-transform:uppercase;padding-bottom:6px'>{label}</div>",
                    unsafe_allow_html=True
                )
            st.markdown("<hr style='border-color:#f0f0f0;margin:0 0 4px 0'>", unsafe_allow_html=True)

            # 현재 페이지 행 렌더링
            for i, v in enumerate(page_items):
                global_i = page * PAGE_SIZE + i
                cat   = v["category"]
                color = CAT_COLOR.get(cat, "#999")
                rcols = st.columns([0.5, 2.5, 3, 1, 1, 2, 1.2])

                # 체크박스
                checked = st.session_state.del_checked.get(v["video_id"], False)
                new_checked = rcols[0].checkbox(
                    "", value=checked,
                    key=f"chk_{global_i}_{v['video_id']}",
                    label_visibility="collapsed"
                )
                if new_checked != checked:
                    st.session_state.del_checked[v["video_id"]] = new_checked
                    st.rerun()

                rcols[1].markdown(
                    f"<div style='font-family:monospace;font-size:0.8rem;color:#555;"
                    f"padding:6px 0'>{v['video_id']}</div>",
                    unsafe_allow_html=True
                )
                rcols[2].markdown(
                    f"<div style='font-size:0.82rem;color:#333;padding:6px 0'>{v['title']}</div>",
                    unsafe_allow_html=True
                )
                rcols[3].markdown(
                    f"<div style='padding:6px 0'>"
                    f"<span style='background:{color}18;color:{color};font-size:0.75rem;"
                    f"font-weight:700;padding:2px 8px;border-radius:10px'>{cat}</span></div>",
                    unsafe_allow_html=True
                )
                rcols[4].markdown(
                    f"<div style='font-size:0.82rem;color:#555;padding:6px 0'>{v['confidence']}</div>",
                    unsafe_allow_html=True
                )
                rcols[5].markdown(
                    f"<div style='font-size:0.8rem;color:#888;padding:6px 0'>{v['created_at']}</div>",
                    unsafe_allow_html=True
                )
                if rcols[6].button("🗑️ 삭제", key=f"del_btn_{global_i}_{v['video_id']}", use_container_width=True):
                    st.session_state[f"del_confirm_{v['video_id']}"] = True

                # 단건 삭제 확인 (인라인)
                if st.session_state.get(f"del_confirm_{v['video_id']}"):
                    st.markdown(f"""
                    <div style="background:#fff5f5;border:1px solid #fca5a5;border-radius:10px;
                                padding:14px 18px;margin:4px 0 10px 0">
                      <b style="color:#dc2626">⚠️ 정말 삭제할까요?</b>
                      <span style="font-family:monospace;font-size:0.85rem;color:#555;margin-left:10px">
                        {v['video_id']}
                      </span><br>
                      <span style="font-size:0.78rem;color:#888">
                        AnalysisResults · Contents 에서 영구 삭제됩니다.
                      </span>
                    </div>
                    """, unsafe_allow_html=True)
                    conf_cols = st.columns([1, 1, 6])
                    if conf_cols[0].button("✅ 확인 삭제", key=f"del_yes_{global_i}_{v['video_id']}", type="primary"):
                        ok = delete_video(v["video_id"])
                        if ok:
                            st.success(f"✅ `{v['video_id']}` 삭제 완료")
                            st.session_state.pop(f"del_confirm_{v['video_id']}", None)
                            st.session_state.del_checked.pop(v["video_id"], None)
                            st.rerun()
                        else:
                            st.error("삭제 실패 — DB 연결을 확인하세요.")
                    if conf_cols[1].button("취소", key=f"del_no_{global_i}_{v['video_id']}"):
                        st.session_state.pop(f"del_confirm_{v['video_id']}", None)
                        st.rerun()

            # 선택 일괄삭제 배너
            selected_ids = [vid for vid, chk in st.session_state.del_checked.items() if chk]
            if selected_ids:
                st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)
                st.markdown(f"""
                <div style="background:#fff5f5;border:1px solid #fca5a5;border-left:4px solid #ef4444;
                  ...
                  <div>
                    <b style="color:#dc2626">⚠️ {len(selected_ids)}개 선택됨</b>
                ...
                """, unsafe_allow_html=True)
                bulk_cols = st.columns([1.2, 1, 6])
                if bulk_cols[0].button(f"🗑️ 선택 {len(selected_ids)}건 삭제", key="btn_bulk_delete", type="primary"):
                    st.session_state.bulk_delete_confirm = True
                if bulk_cols[1].button("선택 해제", key="btn_bulk_clear"):
                    st.session_state.del_checked = {}
                    st.rerun()

                if st.session_state.get("bulk_delete_confirm"):
                    st.warning(f"정말 {len(selected_ids)}건을 삭제할까요? 되돌릴 수 없습니다.")
                    fc1, fc2, fc3 = st.columns([1, 1, 6])
                    if fc1.button("✅ 최종 확인", key="btn_bulk_final", type="primary"):
                        success_cnt, fail_cnt = 0, 0
                        for vid in selected_ids:
                            if delete_video(vid):
                                success_cnt += 1
                                st.session_state.del_checked.pop(vid, None)
                            else:
                                fail_cnt += 1
                        st.session_state.bulk_delete_confirm = False
                        if success_cnt:
                            st.success(f"✅ {success_cnt}건 삭제 완료" + (f" / {fail_cnt}건 실패" if fail_cnt else ""))
                        if fail_cnt and not success_cnt:
                            st.error("삭제 실패 — DB 연결을 확인하세요.")
                        st.rerun()
                    if fc2.button("취소", key="btn_bulk_cancel"):
                        st.session_state.bulk_delete_confirm = False
                        st.rerun()
            ...
            btn_style = (
                "display:inline-block;padding:5px 18px;font-size:0.83rem;font-weight:500;"
                "border-radius:6px;border:1px solid #d1d5db;background:#ffffff;color:#374151;"
                "cursor:pointer;line-height:1.5;text-decoration:none;"
            )
            disabled_style = (
                "display:inline-block;padding:5px 18px;font-size:0.83rem;font-weight:500;"
                "border-radius:6px;border:1px solid #e5e7eb;background:#f9fafb;color:#c0c0c0;"
                "cursor:default;line-height:1.5;text-decoration:none;"
            )
            bp1, bp2, bp3 = st.columns([1, 1, 6])
            with bp1:
                if st.button("◀ 이전", key="del_prev_b", disabled=prev_disabled, use_container_width=True):
                    st.session_state.del_page -= 1
                    st.rerun()
            with bp2:
                if st.button("다음 ▶", key="del_next_b", disabled=next_disabled, use_container_width=True):
                    st.session_state.del_page += 1
                    st.rerun()
            st.markdown("""
            <style>
            /* 하단 페이지네이션 버튼 — 검색바와 동일한 높이·색상 */
