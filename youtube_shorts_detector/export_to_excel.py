...
def export():
    # ✅ 세션 내에서 모든 데이터 미리 추출 (DetachedInstanceError 방지)
    rows = []
    stats = {}
    ...
    headers = ["No", "Video ID", "제목", "채널명", "카테고리", "카테고리명",
               "신뢰도", "상태", "처리시간(초)", "분석일시", "판단 근거"]
    ...
    for row_idx, r in enumerate(rows, 2):
        ...
        row_data = [
            ...
            r["channel_name"],
            r["c_category"],
            CATEGORY_NAMES.get(r["c_category"], ""),
            r["confidence_score"],
            r["status"],
            r["processing_time"],
            r["created_at"],
    ...
    col_widths = [6, 15, 35, 20, 10, 14, 10, 14, 12, 20, 60]
